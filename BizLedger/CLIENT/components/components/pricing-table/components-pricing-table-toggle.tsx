...
type ComponentsPricingTableToggleProps = {
    items?: PricingItem[];
    loading?: boolean;
...
}: ComponentsPricingTableToggleProps) => {
    ...
    const content = (
        <div className="mb-5">
            <div className="mx-auto max-w-[320px] dark:text-white-dark md:max-w-[1140px]">
                ...
                {!loading && items && (
                    <div className="mt-6 grid grid-cols-1 gap-6 text-white-dark md:mt-12 md:grid-cols-2 xl:grid-cols-3">
                        {items.map((item) => {
                            const isHighlight = item.highlight;
                            return (
                                <div
                                    key={item.id}
                                    className={`relative rounded-2xl border border-white-light/70 bg-white/80 p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md dark:border-[#1b2e4b] dark:bg-[#0f172a]/40 ${
                                        isHighlight ? 'ring-1 ring-primary/40' : ''
                                    }`}
                                >
                                    <h3 className="mb-2 text-lg font-semibold text-black dark:text-white-light">{item.title}</h3>
                                    <p className="text-xs uppercase tracking-wide text-gray-400">{item.status || 'active'}</p>
                                    {item.description && <p className="mt-3 text-sm text-gray-500 dark:text-gray-400">{item.description}</p>}
                                    <button
                                        type="button"
                                        className={`btn mt-6 w-full ${isHighlight ? 'btn-primary' : 'btn-outline-primary'}`}
                                        onClick={() => onSelect?.(item.id)}
                                        disabled={!!selectingId && selectingId === item.id}
                                    >
                                        {!!selectingId && selectingId === item.id ? 'Selecting...' : actionLabel}
                                    </button>
                                </div>
                            );
                        })}
